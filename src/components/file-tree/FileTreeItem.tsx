...
const getFileIcon = (fileName: string) => {
  ...
  switch (ext) {
    ...
    case '.ts':
    case '.tsx':
      return <span className="text-blue-500 font-bold text-xs w-5 text-center">JS/TS</span>;
    case '.json':
      return <span className="text-purple-500 font-bold text-xs w-5 text-center">{}</span>;
    case '.md':
    case '.markdown':
      return <span className="text-gray-500 font-bold text-xs w-5 text-center">MD</span>;
    case '.html':
    case '.htm':
      return <span className="text-orange-500 font-bold text-xs w-5 text-center">&lt;/&gt;</span>;
    case '.css':
      return <span className="text-blue-400 font-bold text-xs w-5 text-center">#</span>;
    case '.xml':
      return <span className="text-green-500 font-bold text-xs w-5 text-center">&lt;?</span>;
    case '.py':
      return <span className="text-yellow-600 font-bold text-xs w-5 text-center">PY</span>;
    case '.java':
      return <span className="text-red-500 font-bold text-xs w-5 text-center">JV</span>;
    case '.go':
      return <span className="text-cyan-500 font-bold text-xs w-5 text-center">GO</span>;
    case '.rb':
      return <span className="text-red-700 font-bold text-xs w-5 text-center">RB</span>;
    case '.php':
      return <span className="text-indigo-500 font-bold text-xs w-5 text-center">PHP</span>;
    case '.c':
    case '.cpp':
    case '.h':
      return <span className="text-gray-600 font-bold text-xs w-5 text-center">C++</span>;
    case '.sh':
      return <span className="text-green-700 font-bold text-xs w-5 text-center">SH</span>;
    case '.yaml':
    case '.yml':
      return <span className="text-pink-500 font-bold text-xs w-5 text-center">YML</span>;
    case '.env':
      return <span className="text-gray-700 font-bold text-xs w-5 text-center">.ENV</span>;
    case '.gitignore':
      return <span className="text-gray-400 font-bold text-xs w-5 text-center">.GIT</span>;
    default:
      ...
      {fileEntry.type === 'directory' && isExpanded && fileEntry.children && (
        <Box sx={{ pl: 0 }}>
          ...
          {fileEntry.children.map((child) => (
            <FileTreeItem key={child.filePath} fileEntry={child} projectRoot={projectRoot} />
          ))}
        </Box>
      )}
    </Box>
