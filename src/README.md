...
│   ├── api/            # API client functions for interacting with the backend (e.g., `auth.ts`, `file.ts`, `llm.ts`, `terminal.ts`, `translation.ts` for operations like authentication, file management, LLM requests, terminal commands, and AI translation)
...
│   │   └── ...         # Other general UI components (e.g., `Layout.tsx`, `Navbar.tsx`, `Loading.tsx`, `WelcomeMessage.tsx`, `PromptGenerator.tsx`, `FilePickerDialog.tsx`, `AiResponseDisplay.tsx`, `OpenedFileViewer.tsx`, `ProposedChangeCard.tsx`, `ThemeToggle.tsx`, `Snackbar.tsx`, `RunScriptMenuItem.tsx`)
...
│   │   ├── spotify/    # Components specific to the Spotify-like app (e.g., `SpotifyHomePage.tsx`, `SpotifyPlayerBar.tsx`, `SpotifySidebar.tsx`)
│   │   └── ...         # Other pages (e.g., `AiEditorPage.tsx`, `LoginPage.tsx`, `AuthCallback.tsx`, `RegisterPage.tsx`, `AppsPage.tsx`, `HomePage.tsx`, `DashboardPage.tsx`, `SpotifyAppPage.tsx`, `TranslatorAppPage.tsx`)
...
│   ├── stores/         # Nanostores for centralized, reactive global state management (e.g., `authStore.ts`, `aiEditorStore.ts`, `fileTreeStore.ts`, `themeStore.ts`, `spotifyStore.ts`, `translatorStore.ts`)
