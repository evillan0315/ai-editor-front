...
const SpotifyAppPage: React.FC = () => {
  ...
  return (
    ...
    >
      ...
      {/* Video Overlay - conditionally rendered and absolutely positioned */}
      {currentTrack?.fileType === FileType.VIDEO && currentTrack.mediaSrc && (
        <Box
          sx={{
            gridArea: 'main', // Visually overlaps main content area
            position: 'absolute',
            top: 0,
        ...
        >
          <VideoPlayer
            src={currentTrack.mediaSrc}
            mediaElementRef={mediaElementRef} // Pass the common ref for VideoPlayer to update
            options={videoJsOptions}
            // onReady is not strictly needed here as mediaElementRef is updated directly by VideoPlayer
          />
        </Box>
      )}
